...
class StepperControlLoopTest : public control_loop::StepperControlLoop {
   public:
    StepperControlLoopParams default_params{.i_hold = 0.5, .i_run = 1.0, .kp = 0.1, .ki = 0.1, .kd = 0.1, .max_speed = 10.0};

    hwbridge::MOCK_HBRIDGE bridge_a;
    hwbridge::MOCK_HBRIDGE bridge_b;
    basilisk_hal::MOCK_HAL_CLOCK clock;

    StepperControlLoopTest() : StepperControlLoop(bridge_a, bridge_b, clock) {}
